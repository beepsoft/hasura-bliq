(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('react'), require('graphql'), require('mobx-state-tree'), require('graphql-tag')) :
    typeof define === 'function' && define.amd ? define(['exports', 'react', 'graphql', 'mobx-state-tree', 'graphql-tag'], factory) :
    (factory((global.hasuraBliq = {}),global.react,global.graphql,global.mobxStateTree,global.gql));
}(this, (function (exports,react,graphql,mobxStateTree,gql) {
    ...
    (function (PrepareTarget) {
      PrepareTarget[PrepareTarget["Subscription"] = 0] = "Subscription";
      PrepareTarget[PrepareTarget["Delta"] = 1] = "Delta";
    ...
    function prepareDeltaQuery(query, config) {
    ...
    function prepareSubscriptionQuery(query, config) {
    ...
    function prepareQuery(target, query, config) {
      ...
      if (queryCopy.definitions[0].kind == "OperationDefinition") {
        ...
        if (node.selectionSet.selections[0].kind == "Field") {
          var fieldNode = node.selectionSet.selections[0]; // Everything in a DocumentNode is readonly by TS types, but we hack it
          // around so that we can update values in the AST directly

          var fieldNodeAny = fieldNode; // If there are no arguments yet, add an empty arra

          if (!fieldNode.arguments) {
            fieldNodeAny.arguments = new Array();
          }

          var argumentNodes = fieldNodeAny.arguments;
          var hasLimit = false;
          var hasWhere = false;

          for (var i = 0, list = argumentNodes; i < list.length; i += 1) {
            // In case of subscription query target set limit to 1
            var arg = list[i];

            if (target == PrepareTarget.Subscription && arg.name.value == "limit") {
              arg.value.value = "1";
              hasLimit = true; //console.log("limit node", JSON.stringify(arg, null, 2));
            }
          ...
          if (target == PrepareTarget.Subscription && !hasLimit) {
            ...
            argumentNodes.push(limitEquals1);
    ...
    function prepareWhereClause(where, config) {
      ...
      where.value = ands; //console.log("ands", ands)
    }
    ...
    function useLiveQuery(params) {
      ...
      var gqlWsClient = params.gqlWsClient;
      var gqlHttpClient = params.gqlHttpClient;
      var query = params.query;
      var variables = params.variables;
      var onData = params.onData;
      var config = params.config; if ( config === void 0 ) config = {};
      var startDate = params.startDate; if ( startDate === void 0 ) startDate = hooks().utc().format(); // Set default values
      ...
      var ref = react.useState(startDate);
      var lastStartDate = ref[0];
      var setLastStartDate = ref[1];
      var ref$1 = react.useState(false);
      var finished = ref$1[0];
      var setFinished = ref$1[1];
      var ref$2 = react.useState(params.canStart === undefined ? true : params.canStart);
      var canStart = ref$2[0];
      var setCanStart = ref$2[1]; // Save a ref to an unsubscribe function which can be called indirectly by the
      // invoker of  useLiveQuery() to finish subscriptions

      var subscription = react.useRef({}); // Make sure config contains the lastStartDate we start from
      ...
      react.useEffect(function () {
        ...
        setCanStart(params.canStart === undefined ? true : params.canStart);
        var queryDoc = typeof query === "string" ? gql(query) : query;
        var subsQuery = prepareSubscriptionQuery(queryDoc, config);
        var varsWithStartDate = Object.assign({}, variables);
        varsWithStartDate[config.startDateVariable] = config.startDate;

        if (params.debug) {
          debugLog("Subscribe with ", graphql.print(subsQuery));
        ...
        var sub = gqlWsClient.request({
          ...
          next: function next(data) {
            ...
            var res = getFirstValue(data.data);
            ...
            sub.unsubscribe(); // Run delta query for all changed elements since lastStartDate

            var deltaQuery = graphql.print(prepareDeltaQuery(queryDoc, config));
            debugLog("Executing delta query: ", deltaQuery);
            gqlHttpClient.request(deltaQuery, varsWithStartDate).then(function (res) {
              // console.log("Delta result: ", res)
              var list = getFirstValue(res);
      ...
      }, [canStart, lastStartDate, finished]); // Return info and controls for managing the subscription
    ...
    function useLiveQueryWithStore(params) {
      var debugLog = function (msg, obj) {
        if (params.debug) {
          console.log("-- useLiveQuery: " + msg, obj);
        }
      ...
      var ref = mobxStateTree.getEnv(params.store);
      var gqlWsClient = ref.gqlWsClient;
      var gqlHttpClient = ref.gqlHttpClient;

      var storeHandler = function (data) {
        debugLog("Raw data received: ", data);
        var merged = params.store.merge(getFirstValue(data));
      ...
      var liveQueryInfo = useLiveQuery({
        gqlWsClient: gqlWsClient,
        gqlHttpClient: gqlHttpClient,
        query: params.query,
    ...
    exports.useLiveQueryWithStore = useLiveQueryWithStore;

(function (global, factory) {
    ...
    function useLiveQuery(params) {
      ...
      var setFinished = ref$1[1]; // Save a ref to an unsubscribe function which can be called indirectly by the
      // invoker of  useLiveQuery() to finish subscriptions

      var subscription = react.useRef({});

      if (params.canStart === undefined) {
        params.canStart = true;
      } // Make sure config contains the lastStartDate we start from


      config.startDate = lastStartDate;
      debugLog("config:", config);
      debugLog("canStart:", params.canStart);
      ...
      }, [params.canStart, lastStartDate, finished]); // Return info and controls for managing the subscription
